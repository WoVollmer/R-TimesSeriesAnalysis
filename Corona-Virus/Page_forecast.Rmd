...
forecast_table <- coefs %>% 
  ...
  mutate(T_doubling = (log(2) / beta_1),
         ...
         FC_14days = exp(beta_0 + unclass(last_date + 1 + range_fc_long - 1) 
                         * beta_1)) %>% 
  arrange(Case_Type) %>% 
...
### Germany - Forecast Accuracy Check Daily Rolling Mean Cases for the past `r range_fc_short` days {data-height=225}
...
### Case Fatality Rate (CFR in %) - Trend as of mid March (the first deaths occur) {data-height=250}

```{r plot CFR_past_period and CFR_total}
...
p1 <- p_templ %+% geom_line(aes(y = CFR_past_period)) +
  ylim(0, 10) +
  gghighlight(label_params = list(alpha = 0.8)) +
  ggtitle(paste("CFR past period (in %) with", lag_n, "lag-days"))
