...
```{r plot log10scale w/o date filter, fig.width = 14, fig.asp = 0.8, eval = FALSE}
...
p1 <- p_templ %+% geom_line(aes(y = log10(Confirmed)), size = 1) +
  gghighlight(label_params = list(alpha = 0.8)) +
  ggtitle("Confirmed on log scale")
p2 <- p_templ %+% geom_line(aes(y = log10(Conf_100k)), size = 1) +
  gghighlight(label_params = list(alpha = 0.8)) +
  ggtitle("Confirmed / 100k Inhabitant on log scale")
...
```{r plot daily log10scale w/o date filter, fig.width = 14, fig.asp = 0.8, eval = FALSE}
...
p2 <- p_templ %+% geom_line(aes(y = log10(Mean_Daily_Conf_100k)), size = 1) +
  gghighlight(label_params = list(alpha = 0.8)) +
  ggtitle("Mean Daily Conf / 100k Inhabitant on log scale")
...
p1 <- p_templ %+% aes(y = log10(Mean_Daily_Conf)) +
  geom_line( size = 1) +
  geom_smooth(method="lm", aes(col = Country), lty = "dashed", se=FALSE) +
  gghighlight(label_params = list(segment.alpha = 0.8)) +
  ylab("") +
  ggtitle("Mean Daily Confirmed on log scale")
