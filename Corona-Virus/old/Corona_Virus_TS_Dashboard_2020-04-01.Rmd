...
```{r  Time Series Confirme dygraph_plot}
...
```{r  Time Series Deaths dygraph_plot}
...
gg_logscale <- function(data) {
  gg_plot <-  
    ...
    labs(x = "Date", y = "Cumulated Cases",
         title = 
           "Exponential Growth - World and selected Countries (log10 scale)") + 
...
China and South Korea significantly slowed down exponential growth.
...
```{r plot log10scale selected countries wo time filter}
...
forcast_countries <-
  tibble(Country = countries_to_forecast,
       confirmed_to_death = c(13, 12, 11),
       infected_to_death     = 21,
       case_fatality_rate = c(0.06, 0.3, 0.45)) %>% 
...
```{r eval = TRUE, fig.width = 6, fig.asp = 0.618}
...
for (country in forcast_countries$Country) {
corona_wide <- corona_country_sel %>% 
  filter(Country == country)
lag_n <- filter(forcast_countries, Country == country)$confirmed_to_death
case_fatality_rate <- 
  filter(forcast_countries, Country == country)$case_fatality_rate 
corona_wide %<>%   
...
### Germany - Example plot for Daily Deaths depending on lagged Daily Confirmed Cases
...
fit_arima_lag <- corona_tsbl_lag_range_wide %>%
  ...
  mutate(Deaths = c(rep(NA, 3), Deaths[(4):no_values])) %>%
  # Estimate models
  model(lag_0 = ARIMA(Deaths ~ 0 + pdq(p = 0, d = 0) + lag(Confirmed, lag_days)),
        lag_0_2 = ARIMA(Deaths ~ 0 + pdq(p = 0, d = 0) +
                          lag(Confirmed, lag_days - 2) +
                          lag(Confirmed, lag_days - 1) +
                          lag(Confirmed, lag_days) +
                          lag(Confirmed, lag_days + 1) +
                          lag(Confirmed, lag_days + 2)),
        lag_1 = ARIMA(Deaths ~ 0 + pdq(p = 0, d = 0) + lag(Confirmed, lag_days_1)),
        lag_1_2 = ARIMA(Deaths ~ 0 + pdq(p = 0, d = 0) +
                          lag(Confirmed, lag_days_1 - 2) +
                          lag(Confirmed, lag_days_1 - 1) +
                          lag(Confirmed, lag_days_1) +
                          lag(Confirmed, lag_days_1 + 1) +
                          lag(Confirmed, lag_days_1 + 2)),
        lag_2 = ARIMA(Deaths ~ 0 + pdq(p = 0, d = 0) + lag(Confirmed, lag_days_2)),
        lag_2_2 = ARIMA(Deaths ~ 0 + pdq(p = 0, d = 0) +
                          lag(Confirmed, lag_days_2 - 2) +
                          lag(Confirmed, lag_days_2 - 1) +
                          lag(Confirmed, lag_days_2) +
                          lag(Confirmed, lag_days_2 + 1) +
                          lag(Confirmed, lag_days_2 + 2))
  )
# identify lowest AICc value for best fit
...
```{r corona tsibble ARIMA lag_range, fig.width = 5, fig.asp = 1.2}


# model ~ 0 + => w/o intercept
fit_best <- corona_tsbl_lag_range_wide %>% 
  # filter(Country == country) %>% 
  model(ARIMA(Deaths ~ 0 + pdq(d = 0) +  # pdq(p = 0, d = 0, q = 0) + 
                          lag(Confirmed, lag_days - 2) + 
                          lag(Confirmed, lag_days- 1) +
                          lag(Confirmed, lag_days) +
                          lag(Confirmed, lag_days+ 1) +
...
### Forecast Residuals {data-height=150}
...
Forecast residuals indicate quality of fit with Arima moving range model 
`r coef(fit_best)$.model[1]` .

### Chart Forecast Residuals w/ Correlogram and Histogram
...
fit_best  %>% 
  ...
  labs(title = paste(country, "-  Forecast Residuals w/ Correlogram and Histogram"),
       subtitle = paste("Model: ARIMA(moving average); lag_days:", lag_days))
```
