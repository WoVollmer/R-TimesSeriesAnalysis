...
Trend / Daily Change {data-orientation=rows}
=====================================
...
**`r i` Cumulated Cases / Daily Changes over Time**
...
```{r  Time Series Diagram countries 8}
country <- countries[i]
gg_trend_daily(corona_country %>% filter(Country == country), country)
...
### Comparison Exponential Growth of selected Countries
...
corona_tsbl_world <- corona_world %>% 
  mutate(Country = "World") %>% 
  as_tsibble(index = Date, key = c(Country, Case_Type)) %>% 
  ungroup() %>% group_by(Case_Type) %>% 
  filter(Date >= last_date + 1 - reg_range &
           Case_Type != "Recovered") %>% 
  select(Country, Case_Type, Date, Cases)
...
```{r example world, fig.width = 10, fig.height = 5}

fit_corona_world <- corona_tsbl_world %>% ungroup() %>% 
  group_by(Case_Type) %>% 
  model(TSLM(log(Cases) ~ Date)) 
fc_corona_world <- fit_corona_world %>% fabletools::forecast(h = "14 days") 

plot_world_forecast <- fc_corona_world %>%  
  autoplot(corona_tsbl_world) +
  ggtitle(paste("World", "- Cases and 14-days Forecast on linear y-scale"),
            subtitle = "Example shows the extreme fast increase on linear scale") +
  labs(x = "Days") +
  # scale_y_log10() +
  scale_x_date(date_labels = "%b %d", date_breaks = "7 days") +
...
coefs_world <- tidy(fit_corona_world) %>% 
  group_by(Country, Case_Type) %>% 
  mutate(T_doubling = (log(2) /estimate))
...
forecast_table_world <- forecast_world %>%
  filter(Case_Type != "Recovered") %>% 
  group_by(Country, Case_Type) %>%
  summarise(T_doubling = sum(T_doubling, na.rm = TRUE),
            FC_next_day = sum(FC_next_day, na.rm = TRUE),
...
forecast_table_world <- left_join(forecast_table_world, world_last_day) %>% 
  select(Country, Case_Type, T_doubling, last_day, 
         FC_next_day, FC_14days)
...
knitr::kable(forecast_table_world,  digits = c(0, 0, 1, 0, 0),
             caption = 
             "Forecast (FC) with linear regression: Doubling Time (days), 
             Forecasted cases tomorrow and  Forecasted cases in 14 days")
...
### Check of Forecast Accuracy {data-height=100}
...
Note: as of 2020-03-24 the data files used so far will no longer be updated. 
They have releaseed a new data structure which does no longer provide the 
recovered cases.
