...
highchart() %>%
  ...
  hc_subtitle(text = 'Total Confirmed / Country - Actual Figures')
```


# World Map - Total Deaths for each Country
...
highchart() %>%
  ...
  hc_colorAxis(stops = color_stops()) %>% 
  hc_title(text = "Spread of Coronavirus SARS-CoV-2") %>% 
  hc_subtitle(text = 'Total Deaths / Country - Actual Figures')
```
...
filter(corona_country_last, Case_Type == "Confirmed") %>%
  ...
  hc_title(text = "Total Confirmed - Descending Order") %>% 
...
filter(corona_country_last, Case_Type == "Deaths") %>%
  ...
  hc_title(text = "Total Deaths - Descending Order") %>% 
  hc_add_theme(hc_theme_sandsignika())
```
...
i <- "Worldwide"
plot_cases <- ggts_cases_facet(corona_world) + 
      labs(title = paste(i, "- Cumulated Cases"))
plot_daily_change <- ggts_cases_facet(corona_world, y = Daily_Change) +
...
for (i in countries) {
  plot_cases <- 
    ggts_cases_facet(corona_country %>% filter(Country == i)) +
      labs(title = paste(i, "- Cumulated Cases")) 
  plot_daily_change <- ggts_cases_facet(
    corona_country %>% filter(Country == i), y = Daily_Change) +
            labs(title = paste(i, "- Daily Changes"))
gridExtra::grid.arrange(plot_cases, plot_daily_change, ncol = 2)
}
...
compare_countries <- c(forc_countries, "France", "United States of America")
...
forecast_country <- coefs %>% 
  ...
  mutate(T_doubling = (log(2) / beta_1),
         forecast_next_day = exp(beta_0 + unclass(Sys.Date()) * beta_1),
         forecast_14days = exp(beta_0 + unclass(Sys.Date() + forcast_range-1) 
                              * beta_1)) 
...
forecast_table <- forecast_country %>%
  ...
  summarise(T_doubling = sum(T_doubling, na.rm = TRUE),
            forecast_next_day = sum(forecast_next_day, na.rm = TRUE),
            forecast_14days = sum(forecast_14days, na.rm = TRUE)) %>% 
  arrange(Case_Type)
...
forecast_table <- left_join(forecast_table, countries_last_day) %>% 
  select(Country, Case_Type, T_doubling, last_day, 
         forecast_next_day, forecast_14days)
