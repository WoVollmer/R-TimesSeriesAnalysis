...
corona_confirmed <- read_csv(
  "./novel-corona-virus-2019-dataset/time_series_19-covid-confirmed.csv") %>%
  mutate(Case_Type = "Confirmed")
corona_deaths <- read_csv(
  "./novel-corona-virus-2019-dataset/time_series_19-covid-deaths.csv") %>%
  mutate(Case_Type = "Deaths")
corona_recovered <- read_csv(
...
sum((corona_confirmed %>% filter(`Country/Region` == "US"))$`3/18/20`, na.rm = TRUE)
#> [1] 1670
sum((corona_confirmed %>% filter(`Country/Region` == "US"))$`3/17/20`, na.rm = TRUE)
sum((corona_confirmed %>% filter(`Country/Region` == "US"))$`3/16/20`, na.rm = TRUE)
sum((corona_confirmed %>% filter(`Country/Region` == "US"))$`3/15/20`, na.rm = TRUE)
#> [1] 1281
...
Sys.Date()
View(corona %>% filter(Country == "France" & Date == Sys.Date() - 10))
selection <- corona %>% filter(Country == "France" & Date >= Sys.Date() - 5)
...
corona_country_sum <- corona %>% 
  group_by(Case_Type, Date, Country)  %>% 
...
highchart() %>%
  hc_add_series_map(worldgeojson, 
                    filter(corona_country_last, Case_Type == "Confirmed"), 
                    value = 'Cases', 
...
filter(corona_country_last, Case_Type == "Confirmed") %>%
  ...
  hc_title(text = "Total Confirmed - Top Ten Country") %>% 
...
title <- "Top 10 Country of death"
filter(corona_country_last, Case_Type == "Deaths") %>%
  arrange(desc(Cases)) %>% 
  head(10) %>%
  hchart("bar",hcaes(x = Country,  y = Cases, title = title)) %>%
  hc_title(text = "Total Deaths - Top Ten Country") %>% 
  hc_add_theme(hc_theme_sandsignika())
```
...
i <- "Worldwide"
plot_cases <- ggts_cases_facet(corona_world_sum) + 
      labs(title = paste(i, "- Cumulated Cases"))
plot_daily_change <- ggts_cases_facet(corona_world_sum, y = Daily_Change) +
        labs(title = paste(i, "- Daily Changes"))
gridExtra::grid.arrange(plot_cases, plot_daily_change, ncol = 2)

countries <- c("China", "South Korea", "Austria", "Germany", "Italy", "Spain")
forc_countries <- c("Austria", "Germany", "Italy", "Spain" )
...
for (i in countries) {
  plot_cases <- 
    ggts_cases_facet(corona_country_sum %>% filter(Country == i)) +
      labs(title = paste(i, "- Cumulated Cases")) 
  plot_daily_change <- ggts_cases_facet(
...
# Doubling Time and Forecasting

### For Selected Countries `r forc_countries`
...
For Spain, Italy, Germany we have currently a doubling time $T \approx 9-12 days * 0.3 \approx 2.7 - 4 days !!$. 

## Changes over Time and Linear Regression


```{r plot function with linear regression}
reg_range <- 9   # important no Cases value = 0 are llowed => log(0) = inf !
forcast_range <- 14  # 14days forecast => unclass(Sys.Date() + forcast_range -1)
...
ggts_cases_lm <- function(data, x = Date, y = Cases, col = Case_Type) {
  ...
  ggplot(data, aes_(substitute(x), substitute(y), col = substitute(Case_Type))) +
    facet_wrap(vars(Case_Type), ncol = 1, scales = "free",
               strip.position = "left") +
    geom_line(size = 1, na.rm = TRUE) +  
    geom_smooth(data = data %>% filter(Date >= Sys.Date() - reg_range), 
                method="lm", 
...
### Plot Example with log10 scale

```{r  Plot with Linear Regression, fig.width = 10, fig.height = 6}

sel_country <- "Germany"
# for (i in forc_countries) {
for (i in sel_country) { 
  plot_cases <- 
    ggts_cases_facet(corona_country_sum %>% 
                        filter(Country == i & Case_Type != "Recovered")) +
      labs(title = paste(i, "- Cumulated Cases")) +
    scale_y_log10()
  plot_daily_change <- ggts_cases_lm(
    corona_country_sum %>% 
      filter(Country == i & Case_Type != "Recovered" & Date >= Sys.Date() - 28)) +
            labs(title = paste("w/ linear regression of last", reg_range, "days"))  +
    scale_y_log10() +
...
```{r Comparions of selected Country, fig.width = 10, fig.height = 5}
corona_tsbl <- corona  %>% 
  as_tsibble(index = Date, key = c(Country, `Province/State`, Case_Type))

corona_tsbl_filt <- corona_tsbl %>% ungroup() %>% group_by(Case_Type) %>% 
  filter(Country %in% forc_countries & Date >= Sys.Date() - reg_range &
           Case_Type != "Recovered") %>% 
  select(Country, `Province/State`, Case_Type, Date, Cases)
...
### Plot Example with linear scale showing the extreme forecast increase
...
linear regression over the last ´r forcast_range´ days.
...
fc_corona %>%  
  ...
  ggtitle(paste(sel_country, "- Cases and 14-days Forecast on linear y-scale"),
            subtitle = "Example to show the extreme fast increase on linear scale") +
  labs(x = "Days") +
  # scale_y_log10() +
  scale_x_date(date_labels = "%b %d", date_breaks = "7 days") +
...
## Doubling Time and Forecast next 14 Days

The forcasted expected cases for the next 14 days are calculated 'only' from the
linear regression of the logarithmic data are not considering any effects of 
...
coefs <- tidy(fit_corona) %>% 
  group_by(Country, `Province/State`, Case_Type) %>% 
  mutate(T_doubling = (log(2) /estimate))

forecast_country <- coefs %>% 
  pivot_wider(id_cols = c(Country, `Province/State`, Case_Type), 
              names_from = term, 
              values_from = estimate) %>% 
  rename(beta_0 = `(Intercept)`,
         beta_1 = Date) %>% 
  mutate(T_doubling = (log(2) / beta_1),
         forcast_tomorrow = exp(beta_0 + unclass(Sys.Date()) * beta_1),
         forcast_14days = exp(beta_0 + unclass(Sys.Date() + forcast_range-1) 
                              * beta_1)) 
...
forecast_table <- forecast_country %>%
  ...
  summarise(T_doubling = sum(T_doubling, na.rm = TRUE),
            forcast_tomorrow = sum(forcast_tomorrow, na.rm = TRUE),
            forcast_14days = sum(forcast_14days, na.rm = TRUE)) %>% 
  arrange(Case_Type)

knitr::kable(forecast_table,  digits = c(0, 0, 1, 0, 0),
             caption = "Forecast with linear Regression: Doubling Time (days), 
             Expected cases tomorrow and in 14 Days")

# check
# corona %>%  filter(Country %in% forc_countries, Date >= Sys.Date() - 1) 

```



### Check Forecast Accuracy for last 3 days with Training/Test Data

The Forecast accuracy procedure uses forecast evaluation method three days 
before today and compares the forecast result for the last three days with 
the real data of these days.
...
#    Date >= Sys.Date() - training_data_start = last 3 days

corona_tsbl_train <- corona_tsbl %>% ungroup() %>% group_by(Case_Type) %>% 
  filter(Country %in% sel_country & 
           Date >= Sys.Date() - training_data_start &
           Date < Sys.Date() - training_data_end &
           Case_Type != "Recovered") %>% 
  select(Country, `Province/State`, Case_Type, Date, Cases)
