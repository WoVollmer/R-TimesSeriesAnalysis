...
page_4 <- paste("Case Fatality Rate (CFR) of past", range_cfr, 
                "days assuming a time lag of", lag_n, 
...
```{r Top n Bar Chart CFR selected countries}
...
DT::datatable(
  corona_data_last_wide %>% 
    dplyr::filter(Country %in% c(countries_eu, "EU")) %>% 
    mutate("Population /m" = round(Population / 1000000, digits = 3)) %>% 
    dplyr::select(
      Country, `Population /m`, Date, 
      Confirmed, Daily_Conf_Mean, Conf_100k, Daily_Conf_100k_Mean,
