...
# or absolute
# setwd("D:/Wolfgang/Programs-R/R-TimeSeriesAnalysis/Corona-Virus") # "desktop"
# # attention: here I need (whyever) TimesS... (Time + plural-s) not TimeS...
# setwd("C:/Wolfgang/Programs-R/R-TimesSeriesAnalysis/Corona-Virus") # "notebook"
...
hc_world_map_plot(
  dplyr::filter(corona_data_last, Case_Type == case_type & Country != "World"), 
  value = "Daily_Cases_100k_Mean", title = title)
...
hc_world_map_plot(
  dplyr::filter(cfr_table, Country != "World"), 
...
mean number of cases over the past seven day (*_Mean) are added.
...
mean number of cases over the past seven days (*_Mean) are added.
...
case_type <- "Confirmed"
abscissa <- "Daily_Cases_100k_Mean"
title <- paste(case_type , 
...
hc_bar_chart_country(cfr_table, y = abscissa, title = title, n = 15)
...
knitr::kable(dplyr::filter(cfr_table, Country %in% compare_countries),
             digits = 2)
...
# gg_plot <- ggts_logscale(corona_tsbl_range_reg)  +
...
p <- ggts_conf_deaths_facet(data, y = Daily_Cases_Mean, vars_1 = Case_Type) +
