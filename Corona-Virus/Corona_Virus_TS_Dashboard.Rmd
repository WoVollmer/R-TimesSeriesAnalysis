...
# wide format: 
#    Country Population Date  Confirmed Daily_Confirmed ... Death ...
# for easy to read datatable outputs e.g.with filter for last date
corona_all_wide <- corona_population %>% 
...
# View(corona_all_wide %>% filter(Date == last_date))
```
...
corona_confirmed_lag <- corona_all_wide %>% 
  dplyr::select(!contains("_100k"), -Population, 
...
fit_corona <- corona_range_reg_long_tsbl %>%  
  model(TSLM(log(Cases_rol_mean) ~ Date)) 
...
gg_plot <- gg_logscale(corona_range_reg_long_tsbl, y = Cases_rol_mean)  +
  geom_smooth(method="lm", aes(col = Country), lty = "dashed", se=FALSE)
  ...
  plot_daily_cases <- 
    ggts_cases_facet(corona_population %>% 
                       filter(Country == i & Case_Type != "Recovered" & 
                                Cases >= 10 &
                                Date >= last_date - 28 + 1)) +
    labs(title = paste("w/ linear regression of log data for past", 
...
fit_daily_corona <- corona_range_reg_long_tsbl %>%
  group_by(Case_Type) %>% 
  model(Exp_Growth = TSLM(log(Cases_rol_mean) ~ Date),
        Linear_Growth = TSLM(Cases_rol_mean ~ Date)) 
...
fc_daily_corona %>%  
  ...
  autoplot(corona_range_reg_long_tsbl) +
  labs(title = paste(
    country_select, " - Forecast of Daily Rolling Mean Cases for the next ", 
    forecast_range, "-days"),
    subtitle = paste(
      "Forecast is based on regression of past ",
      range_reg_long, " days")) +
...
countries_last_day <- corona_population %>% 
  filter(Country %in% c(compare_countries, "World") & 
           Date == last_date) %>%
  rename(last_day = Cases_rol_mean) %>% 
  arrange(Case_Type) %>% 
...
```{r cumulated cases forecast coef beta_0 and beta_1, eval = FALSE}
...
knitr::kable(forecast_table,  digits = c(0, 0, 1, 0, 0),
             format.args = list(big.mark = "'"),
             caption = 
             "Forecast (FC) Cumulated Cases: Doubling Time (days), 
             Forecasted cases next day/tomorrow and  Forecasted cases in 14 days")
...
fc_corona_sel %<>% as_tibble(fc_corona_sel) %>% 
  dplyr::select(Date, Country, Case_Type, Cases_rol_mean)
...
p <- ggts_conf_deaths_facet(data, y = Daily_Cases, col = Case_Type)
p # ggplotly(p)
```
