...
```{r source function abd population data}
...
i <- "Confirmed"
bar_chart_countries_hot_spots(filter(corona_population_last, Case_Type == i), i) 
                      #  value = "Cases_100k_rol_mean")
...
for (i in country_select) { 
  plot_cases <- 
    ggts_cases_facet(corona_population %>% 
  ...
  plot_daily_cases <- 
    ...
    geom_smooth(data = corona_population %>% 
                  filter(Country == i & Case_Type != "Recovered" & 
...
countries_last_day <- corona_population %>% 
...
corona_country_sel <- corona_population %>% 
...
data <- corona_population %>%  filter(Country %in% countries_to_forecast &
           Date >= last_date - 21)
