...
##                                                                            ##
##                                          Wolfgang Vollmer, April 15, 2020  ##
## URL:                                         First Version: March 12, 2020 ##
...
>>>>>>> 6d67199e325d7f1663addc93053789bc57dde4f1
...
range_reg <- 7   # important - no Cases value = 0 are llowed => log(0) = inf !
...
source("./ggts_corona.R") # ggplot2 functions for time series plots
=======
```{r path on notebook, eval = TRUE}

corona_confirmed <- read_csv("C:/Wolfgang/Programs-R/COVID-19/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_global.csv") %>%
  mutate(Case_Type = "Confirmed")
corona_deaths <- read_csv("C:/Wolfgang/Programs-R/COVID-19/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv")  %>%
  mutate(Case_Type = "Deaths")
corona_recovered <- read_csv("C:/Wolfgang/Programs-R/COVID-19/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_recovered_global.csv") %>%
  mutate(Case_Type = "Recovered")
```
>>>>>>> 6d67199e325d7f1663addc93053789bc57dde4f1

# read world population for normalization per 100k inhabitants
population_countries <- readRDS("./world_population.RDS")
...
```{r add mean past 7 days}
# add one-sided roll_mean
span <- 7 #  mean over 7 days
  ...
  mutate(Conf_mp7d = round(
    ...
    Deaths_mp7d = round( 
      stats::filter(Daily_Deaths, 
                    filter = rep(1 / span, span), sides = 1), digits = 0)) 
# corona_country_wide %>% filter(Country == "Germany" & Date >= (last_date-6)) %>% 
#        select(Daily_Confirmed) %>% summarize(mean = mean(Daily_Confirmed))

corona_population_wide %<>% 
  mutate(Conf_100k_mp7d = round(
    stats::filter(Daily_Conf_100k, 
                  filter = rep(1 / span, span), sides = 1), digits = 2),
    Deaths_100k_mp7d = round( 
      stats::filter(Daily_Deaths_100k, 
                    filter = rep(1 / span, span), sides = 1), digits = 2)) 
# corona_population_wide %>% filter(Country == "Germany" & Date >= (last_date-6)) %>%
#        select(Daily_Conf_100k) %>% summarize(mean = mean(Daily_Conf_100k))

```

```{r slider alternative rolling mean calculation, eval = FALSE}
corona_country_wide %<>% 
  mutate(Conf_mp7d_s = round(
    slider::slide_dbl(Daily_Confirmed,  ~ mean(.), 
                       .before = span - 1, .after = 0, .complete = TRUE), 
    digits = 0),
    Deaths_mp7d_s = round( 
    slider::slide_dbl(Daily_Deaths,  ~ mean(.), 
                       .before = span - 1, .after = 0, .complete = TRUE), 
    digits = 0))
...
Population numbers are taken from [[@CIA2020]](#bib), [[@CIA2020a]](#bib).
...
The simple calculation with unlagged cumulated confirmed cases divided by cumulated
deaths results in a significant underestimation of the CFR in health systems with early disease detection.
...
The "old" infected cases are at the end of the doubling period no longer infectious (active). This results in a constant infection rate with basic reproduction number $R_0 \sim 1$ or even $<1$.
