...
Country Table overview {data-navmenu="Bar Chart"}
=====================================
...
### Table Confirmed and Deaths - Cumulated and mean daily Cases, overall and  per 100,000 Inhabitants


Population numbers are taken from [[@UNO2020]](#bib).
In order to compensate for daily fluctuations, the mean number of cases for the 
past seven days (*_mp7d) is used instead of the daily cases.
...
DT::datatable(corona_all_data_wide %>% 
                filter(Date == last_date) %>% 
                mutate("Population /m" = round(Population / 1000000, digits = 3)) %>% 
               # dplyr::select(!contains("Daily"), -Population), 
               dplyr::select(Country, `Population /m`, Date, 
                             Confirmed, Conf_mp7d, Conf_100k, Conf_100k_mp7d,
                             Deaths, Deaths_mp7d, Deaths_100k, Deaths_100k_mp7d), 
              options = list(pageLength = 50))
