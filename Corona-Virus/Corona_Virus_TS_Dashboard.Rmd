...
# slide_dbl() => rolling mean double value, centered since .before = .after
corona_population %<>% 
  group_by(Country, Case_Type) %>% 
  mutate(Cases_rol_mean =  round(
    slider::slide_dbl(Daily_Cases,  ~ mean(.),
                      .before = q, .after = q, .complete = TRUE),
    digits = 2),
    Cases_100k_rol_mean = round(
...
# for last date rolling mean before is needed, otherwise all are NA
corona_population_last <- corona_population %>% 
  group_by(Country, Case_Type) %>% 
  mutate(Cases_rol_mean =  round(
    slider::slide_dbl(Daily_Cases,  ~ mean(.),
...
corona_population_last_wide <- corona_population_last %>% 
  pivot_wider(names_from = Case_Type, 
              values_from = c(Cases, Daily_Cases, 
                              Cases_100k, Daily_Cases_100k, 
                              Cases_rol_mean, Cases_100k_rol_mean)) %>% 
...
corona_last_abs_wide <- corona_population_last_wide %>% 
  dplyr::select(!contains("_100k"), -Population)

corona_last_rel_wide <- corona_population_last_wide %>% 
  dplyr::select(Country, Date, contains("_100k"))
```
...
i <- "Confirmed"
title <- paste(i, "- Cumulated Cases / Country - Actual Figures")
world_map_plot(
  filter(corona_population_last, Case_Type == i & Country != "World"), 
  i, value = "Cases", title)
...
title <- 
  paste(i, 
        "- Mean Cases per 100k Inhabitants / Country over the past seven days")
world_map_plot(
  filter(corona_population_last, Case_Type == i & Country != "World"), 
  i, value = "Cases_100k_rol_mean", title)
...
i <- "Confirmed"
bar_chart_countries(filter(corona_population_last,
...
daily fluctuations, the mean case numbers for the past seven days (*_rol_mean) were also added.

```{r print datatable in wide format}

# DT::datatable(corona_population_last %>% filter(Case_Type != "Recovered"),
...
In order to compensate for the daily fluctuations, the mean case numbers for the past seven days (*_rol_mean) were also added.
...
corona_confirmed_lag <- corona_abs_wide %>% 
...
past seven days (*_rol_mean) is used instead of the daily cases.
...
DT::datatable(corona_population_last_wide %>% 
               ...
               dplyr::select(Country, `Population /m`, Date, 
                             Confirmed, Conf_rol_mean, Conf_100k, Conf_100k_rol_mean,
                             Deaths, Deaths_rol_mean, Deaths_100k, Deaths_100k_rol_mean), 
              options = list(pageLength = 50))
...
ggts_trend_daily(corona_population %>% filter(Country == country), 
                 country, weeks = weeks)
...
country <- countries[i]
ggts_trend_daily(corona_population %>% filter(Country == country), country, weeks)
