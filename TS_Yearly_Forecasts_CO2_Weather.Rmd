...
measure_list <- c("CO2", "Temperature", "Precipitation")

city <- c("Basel")
measure <- c("Temperature")
...
n_mismatches <- sum(ifelse(measure %in% measure_list, 0, 1))
if (n_mismatches > 0) { 
  abort(paste(paste(measure, collapse = ", "), 
              "does not match to defined list of allowed cities:", 
              paste(measure_list, collapse = ", ")))
}
if (length(measure) > 1 & "CO2" %in% measure) { 
  abort(paste(paste(measure, collapse = ", "), 
              "- support of 'CO2' and 'Temperature' or 'Precipitation' is not implemented")) 
} else if (length(measure) > 1) { 
  measure <- paste(measure, collapse = " and ")
  } 
...
if (city == "Mauna Loa") {
  title_rmd <- paste(city, "- Atmospheric Carbon Dioxide Data")
  ...
  if (measure == "Temperature and Precipitation") {
    ...
    data_monthly %<>% filter(Measure == measure)
    data_yearly %<>% filter(Measure == measure)
  }
} else {
  abort("City ", city, " not valid, no *.rds data file exists !!")
}  

assert_tibble(data_monthly, min.rows = 1)
assert_tibble(data_yearly, min.rows = 1)
...
# forecast residuals
# fit_ets_all <- dplyr::select(fit_ets_all, City, Measure, ETS_AAN, ETS_AAA, ETS_AAdA)
