...
aus_exports <- global_economy %>%  # slide_db => rolling mean, same result
  filter(Country == "Australia") %>%
  mutate(
    `5-MA` = tsibble::slide_dbl(Exports,  ~ mean(.), .size = 5),
    q_ma_middle =  slider::slide_dbl(Exports,  ~ mean(.), 
                                     .before = q, .after = q, .complete = TRUE),
    q_ma_left =  slider::slide_dbl(Exports,  ~ mean(.), 
                                   .before = 2*q, .after = 0, .complete = TRUE),
    q_ma_right =  slider::slide_dbl(Exports,  ~ mean(.), 
                                    .before = 0, .after = 2*q, .complete = TRUE),
...
# aus_exports_tile <- global_economy %>%  # tile_db => slot by slot => only few data
#   filter(Country == "Australia") %$%
#   slider::slide_dbl(Exports,  ~ mean(.), .before = q, .after = q)

p <- aus_exports %>%
  autoplot(Exports) +
  autolayer(aus_exports, `5-MA`, color='red') +
  autolayer(aus_exports, q_ma_middle, color='cyan') +
    autolayer(aus_exports, q_ma_left, color='cyan1') +
    autolayer(aus_exports, q_ma_right, color='cyan2') +
