...
if (city != "Mauna Loa" & city != "Basel_Giessen") {
  ...
  file <- paste0("D:/Wolfgang/Programs-R/R-TimeSeriesAnalysis",
                 "/WeatherAnalysis/Data_Weather/Weather-Data-Input_", 
                 city, ".csv")
...
# !! next line changes tsibble => tibble !! => as_tsibble() needed afterwards
...
last_ref_year <- start_last_period -1
data_monthly_over_period <- data_monthly_over_period  %>% 
  group_by(Month, Measure) %>% 
  mutate(count_minus_ref = count - count[Period == last_ref_year]) 
...
period_years <- c(paste(first_year, "-", end_first_period, sep = ""),
          paste(start_last_period - span, "-", start_last_period -1, sep = ""),
...
if (length(key(data_monthly)) == 1) { 
  for (i in unique(key_data(data_monthly)[[1]])) {     # City
    ...
    print(plot_monthly + coord_fixed(ratio = ratio_values / ratio_display))
  ...
  for (i in unique(key_data(data_monthly)[[1]])) {     # City
    ...
    plot_monthly <- ggts_year_over_month(data_filter, period = Period,
      ...
      labs(y = y_label_measure[i], 
           title = paste0(title, " - Delta to ", period_years["ref"]), 
           subtitle = subtitle)
    ratio_display <- 4 # ratio x / y
    ratio_values <- 12/
      (max(data_filter$count_minus_ref)-min(data_filter$count_minus_ref))
    print(plot_monthly + coord_fixed(ratio = ratio_values / ratio_display))
