...
# setwd("D:/Wolfgang/Programs-R/WeatherAnalysis")
...
if (city != "Mauna Loa" & city != "Basel_Giessen" & city != "Mannheim") {
  data_monthly_wide <- 
    readRDS(paste0("../WeatherAnalysis/WeatherData_", city, ".rds")) %>% 
    mutate(City = city) %>% 
...
last_ten_years <- TRUE
if (last_ten_years) {
 x_min <- data_plot %$% max(Year_Month) - freq*years + 1 # yearmonth(last_year - years + 1)
...
```{r monthly plot, eval = TRUE, fig.width= 7, fig.height= 4}
...
ggts_season_w_smooth(data = data_monthly) + 
  ...
  labs(y = y_label, 
       subtitle = paste(min(data_monthly$Year_Month), " - ", 
                        max(data_monthly$Year_Month)))
...
if (n_keys(data_monthly) == 1 | length(key(data_monthly)) == 2) { 
  for (i in unique(key_data(data_monthly)[[1]])) {     # City
    for (j in unique(key_data(data_monthly)[[2]])) {   # Measure
      ...
      data_filter <- filter(data_monthly_over_period, City == i, Measure == j)
      plot_monthly <- ggts_year_over_month(data_filter, period = Period) +
        labs(y = y_label, title = paste(title, "-", i, j), subtitle = subtitle)
...
if (n_keys(data_monthly) == 1 | length(key(data_monthly)) == 2) { 
  for (i in unique(key_data(data_monthly)[[1]])) {
    data_monthly_new <- tibble()  
    for (j in unique(key_data(data_monthly)[[2]])) {
      data_ts <- filter(data_monthly, City == i & Measure == j) %$% 
      ...
      print(decomp_plot + 
              labs(title =  paste("Seasonal Decomposition by Loess - ", i, j),
  ...
  for (i in unique(key_data(data_monthly)[[1]])) {     # City
    for (j in unique(key_data(data_monthly)[[2]])) {   # Measure
      ...
      plot_lag <- filter(data_monthly, year(Year_Month) >= year_filter & 
        ...
        labs(x = y_label_measure[j], y = "lag(x, n)",
             col = "Month",
             title ="Lag by n months - y(t) plotted against y(t-n)",
             subtitle = paste(i, j, " - as of year", year_filter)) 
print(plot_lag)
    }
  }
} else {
  ...
  for (i in unique(key_data(data_monthly)[[1]])) {
    for (j in unique(key_data(data_monthly)[[2]])) {
      plot_ts_spec <- data_monthly %>% 
        filter(year(Year_Month) >= year_filter & City == i & Measure == j) %>%  
        gg_tsdisplay(count, plot_type = "spectrum", lag_max = 24) + 
