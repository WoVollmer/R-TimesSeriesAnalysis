...
periods <- data_yearly %>% ungroup() %>% select(Period_Time) %>% unique()
...
### CLINO - Climatological normal (defined by WMO, Normalperiode)
ref_period_start <- 1991   # Reference Period WMO: 1961 - 1990
                           # Begin 2021: Ref Period will change to 1991 - 2020
# if (last_year >= 2025) ref_period_start <- 1991
ref_period_end <- ref_period_start + period - 1
ref_period_old_start <- ref_period_start - 30
ref_period_old_end <- ref_period_end - 30  

last_period_end <- last_year      # last year with December values
last_period_start <- last_period_end - period + 1

ref_period <-  paste0(ref_period_start, "-", ref_period_end)
ref_period_old <-  paste0(ref_period_old_start, "-", ref_period_old_end)
...
data_monthly_period  <- as_tibble(data_monthly) %>% 
  separate(Period_Time, c("Period_start", "Period_end")) %>% 
  mutate(Period_start = as.integer(Period_start),
         Period_end = as.integer(Period_end)) %>% 
...
data_monthly_ref_period <- data_monthly_period %>% 
  filter(Period_end == ref_period_end) %>% 
  group_by(City, Measure, Month, Period_end) %>% 
  dplyr::summarise(count = mean(count, na.rm = TRUE)) %>% 
  ungroup() %>% 
...
```{r mean data table of past time periods old, eval = FALSE}

# min_year <-  tibble(data_yearly) %>% group_by(City, Measure) %>% summarise(Min_Year = min(Year))
# max_of_min_year <- min_year %$% max(Min_Year)
# 
# periods <- tibble(Period_Start = seq.int(ref_period_start+period, max_of_min_year, -period)) %>% 
#   arrange(Period_Start) %>% 
#   mutate(Period_End = Period_Start + period -1)
# periods$Period_End[nrow(periods)] <- last_year
# periods <- periods %>% mutate(Period_Time = paste0(Period_Start, "-", Period_End))
#   
# 
# periods_mean <- matrix(nrow = nrow(periods), ncol =  measure_nr)
# for (i in 1:nrow(periods)){
#   z <- tibble(data_monthly) %>% group_by(City, Measure) %>% 
#       filter(Year >= periods$Period_Start[i] & Year < periods$Period_End[i]) %>% 
#       summarise(Mean = mean(count)) %>% pull()
#   periods_mean [i,] <- z
# }
# 
# periods_mean <- periods_mean %>% as_tibble() %>% rename_with(~measure)
# mean_period_data <- bind_cols(periods$Period_Time, periods_mean) %>% 
#   rename(Period_Time = ...1)
# 
# 
# knitr::kable(mean_period_data, digits = 1, 
...
data_yearly_period <- data_monthly_period %>%  
  group_by(City, Measure, Period_end) %>% 
  dplyr::summarise(count = mean(count, na.rm = TRUE)) 
...
data_yearly_period  <- 
  left_join(data_yearly_period, 
            data_yearly_ref_period %>% select(-Period_end), 
            by = c("City",  "Measure")) %>% 
  group_by(City, Measure) %>% 
  mutate(count_minus_ref = count - count_ref_period) 
...
```{r further period settings, eval = FALSE}
# eval = eval_visualization
# ref_period_start <- 1991
# ref_period_end <- ref_period_start + 29
# 
# periods <- period_years <- periods
...
data_monthly %>% 
  ...
  # facet_grid(vars(!!!key(data_monthly)), scales = "free_y")
...
###  forecast(fit, h = n|"x years")

* forecast(fit, h = x) |> hilo() 
    + converts forecast distributions into intervals. Default: 80% & 95% prediction intervals
* accuracy(forecast h on "data_train", "data")
...
fc_data_train %>%
  ...
  labs(y = "Count CO2 / Temp / Precip", 
       ...
       subtitle = "w/ Forecast on trainings data set comparing w/ real data") +
...
* Forecast Accuracy Evaluation w/ training data "data_train" & test data "data_test"
    ...
    + "data_train" = "data" - forecast_range/"data_test" : data used to train the model (~80% of "data)
