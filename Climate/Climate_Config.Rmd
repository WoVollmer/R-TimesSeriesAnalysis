...
library(ggtime) # gg_subseries() was soft deprecated in feasts 0.4.2. 
                # => Please use ggtime::gg_subseries() instead.
...
if (city == "Mauna Loa") {
  ...
  season_window <- 11     # odd, Temperature:  s.window=31, t.window=19
  trend_window  <- 21 # 41
  ...
  measure <- str_to_title(match.arg(temp_precip_selected, temp_precip_both))
  span <- 30  # width of the rolling window in years, CO2: => 7 years
  season_window <- 11  # odd, seasonal structure long running  # Precip to be checked
  trend_window <- 21 # 19   # to allow more rapid changes    
  
  if (measure == "Both") {
    measure <- "Temperature and Precipitation"
    y_label <- expression(paste("Precipitation / mm/Month    ", 
                                "   Temperature / ", degree*C))
    y_label_inv <-  expression(paste("Temperature / ", degree*C, 
...
```{r eval = FALSE}
# settings beore manual run

city <-  "Mauna Loa" # Basel" # "Giessen" #
temp_precip_selected  <- "both"
```
