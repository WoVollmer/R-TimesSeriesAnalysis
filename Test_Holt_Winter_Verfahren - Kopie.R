...
data_ets_add_mult <- data_model %>%  model(
  add_ANN = ETS(count ~ error("A") + trend("N") + season("N")),    
  add_ANA = ETS(count ~ error("A") + trend("N") + season("A")),    
  add_AAA = ETS(count ~ error("A") + trend("A") + season("A")),      
...
data_ets_add_mult %>% 
  ...
  gg_tsresiduals()
data_ets_add_mult %>% 
  filter(Temp_Precip == "Precipitation") %>% 
  gg_tsresiduals()
...
# level: => no conf level
data_forec %>%
  autoplot(data_model, level = NULL) +
...
# if needed filter also w/ & Temp_Precip == "Temperature"
as_tibble(data_monthly) %>%
  filter(Year_Month >= yearmonth("2015")) %>%
...
filter(data_forec,
       Year_Month == yearmonth("2020 Nov") |
         Year_Month == yearmonth("2020 Dec"))
...
filter(data_forec,
       Year_Month == yearmonth("2034 Nov") |
         Year_Month == yearmonth("2034 Dec"))
